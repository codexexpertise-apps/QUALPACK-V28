...
function adminV28FirstStartLine() {
  ...
  return {
    ligne: lines && lines.length ? String(lines[0] || '').trim() : '',
    detecteur: dets && dets.length ? String(dets[0] || '').trim() : ''
  };
}

function adminV28GetStartProductContext() {
  const clientNom = adminV28FirstStartClientName();
  const lineCtx = adminV28FirstStartLine();
...
async function adminV27AddProduit() {
  ...
  const ctx = adminV28GetStartProductContext();
