...
async function adminV28FirstStartClientName() {
  // 1) Priorité aux éléments START créés dans cette session / stockés localement.
  const rows = adminV27GetStartRows('clients');
  const found = (rows || []).find(r => !adminV27IsHidden('clients', r.label || r.nom));
  if (found) return String(found.label || found.nom || '').trim();

  // 2) Fallback catalogue local opérationnel.
  const names = adminV27GetClientNames();
  if (names && names.length) return String(names[0] || '').trim();

  // 3) Fallback Supabase : utile après reset, rechargement, changement d'appareil ou cache vidé.
  try {
    const res = await fetch(`${SB_URL}/rest/v1/clients?select=nom&${qpSiteEqParam()}&order=nom.asc&limit=1`, { headers: SB_HDR });
    if (res.ok) {
      const data = await res.json();
      if (data && data[0] && data[0].nom) return String(data[0].nom || '').trim();
    }
  } catch (e) {
    console.warn('adminV28FirstStartClientName fallback Supabase ignored:', e);
  }

  return '';
}

async function adminV28FirstStartLine() {
  // 1) Priorité aux éléments START créés dans cette session / stockés localement.
  const rows = adminV27GetStartRows('lignes');
  const found = (rows || []).find(r => !adminV27IsHidden('lignes', r.label || r.nom));
  if (found) {
    return {
      ligne: String(found.label || found.nom || '').trim(),
      detecteur: String(found.detecteur || found.meta || '').trim()
    };
  }
  ...
  if (lines && lines.length) {
    return {
      ligne: String(lines[0] || '').trim(),
      detecteur: dets && dets.length ? String(dets[0] || '').trim() : ''
    };
  }

  // 3) Fallback Supabase : utile après reset, rechargement, changement d'appareil ou cache vidé.
  try {
    const res = await fetch(`${SB_URL}/rest/v1/lignes?select=nom,detecteur_defaut&${qpSiteEqParam()}&order=nom.asc&limit=1`, { headers: SB_HDR });
    if (res.ok) {
      const data = await res.json();
      if (data && data[0] && data[0].nom) {
        return {
          ligne: String(data[0].nom || '').trim(),
          detecteur: String(data[0].detecteur_defaut || '').trim()
        };
      }
    }
  } catch (e) {
    console.warn('adminV28FirstStartLine fallback Supabase ignored:', e);
  }

  return { ligne: '', detecteur: '' };
}

async function adminV28GetStartProductContext() {
  const clientNom = await adminV28FirstStartClientName();
  const lineCtx = await adminV28FirstStartLine();

  return {
    clientNom,
    ligne: lineCtx.ligne,
    detecteur: lineCtx.detecteur
  };
}
...
async function adminV27AddProduit() {
  ...
  const ctx = await adminV28GetStartProductContext();
  const clientNom = ctx.clientNom;
  const ligne = ctx.ligne;
  const detecteur = ctx.detecteur;

  if (!clientNom) {
    toast('Ajoutez d’abord un client START', 'err');
    adminV27SetStatus('Ajoutez d’abord un client avant d’ajouter un produit.', 'err');
    return;
  }
  if (!ligne) {
    toast('Ajoutez d’abord une ligne START', 'err');
    adminV27SetStatus('Ajoutez d’abord une ligne / détecteur avant d’ajouter un produit.', 'err');
    return;
  }
  ...
  try {
    ...
    // V28 START : produit simplifié côté interface.
    // Le client, la ligne et le détecteur START déjà créés sont rattachés automatiquement.
    // La sauvegarde Supabase reste identique.
    ...
    ['admin-v27-prod-nom','admin-v27-prod-qn','admin-v27-prod-tare','admin-v27-prod-qte']
      .forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });

    await adminV27ReloadAfterChange();
    toast('Produit ajouté ✓', 'ok');
    adminV27SetStatus(`Produit ajouté et rattaché à ${clientNom}${ligne ? ' / ' + ligne : ''}.`, 'ok');
  } catch (e) {
